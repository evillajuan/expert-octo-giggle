      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <a href="#home" className="flex items-center gap-2 group">
            ...
            <span className="font-display text-2xl text-[#4A3728]">
          ...
          <div className="hidden md:flex items-center gap-8">
            {links.map(l => (
              <a
                key={l.label}
                href={l.href}
                className="font-round font-700 text-[#8B6E5A] hover:text-rose transition-colors duration-150 text-[15px] font-semibold"
              >
                {l.label}
              </a>
            ))}
          ...
          <div className="md:hidden pb-4 pt-2 border-t border-blush/50">
            {links.map(l => (
              <a
                ...
                className="block py-3 px-2 font-round font-semibold text-[#8B6E5A] hover:text-rose transition-colors"
              >
                {l.label}
              </a>
            ))}
            <a href="#create" className="btn-primary block text-center mt-3">
              ✨ Create Your Book
            </a>
          </div>
        )}
